...
The general formula to find its roots (x-values for which y = 0)
...
def convert_to_int_if_possible(num):
...
def quadratic_roots(a: int, b: int, c: int) -> Iterable[Union[int | float] | str]:
    ...
    x_1 = (-b + d ** 0.5) / (2 * a)
    x_1 = convert_to_int_if_possible(x_1)

    x_2 = (-b - d ** 0.5) / (2 * a)
    x_2 = convert_to_int_if_possible(x_2)

    roots = sorted([x_1, x_2], reverse=True)
    return roots
